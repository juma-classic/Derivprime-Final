define(["exports"], (function(t) {
    ...
    function R(t) {
        return t !== IDBDatabase.prototype.transaction || "objectStoreNames" in IDBTransaction.prototype ? (m || (m = [IDBCursor.prototype.advance, IDBCursor.prototype.continue, IDBCursor.prototype.continuePrimaryKey])).includes(t) ? function(...e) {
            return t.apply(I(this), e), E(y.get(this))
        } : function(...e) {
            return E(t.apply(I(this), e))
        } : function(e, ...s) {
            const n = t.call(I(this), e, ...s);
            return v.set(n, e.sort ? e.sort() : [e]), E(n)
        }
    }
    ...
    function x(t) {
        return "function" == typeof t ? R(t) : (t instanceof IDBTransaction && function(t) {
            ...
            g.set(t, e)
        }(t), e = t, (p || (p = [IDBDatabase, IDBObjectStore, IDBIndex, IDBCursor, IDBTransaction])).some((t => e instanceof t)) ? new Proxy(t, q) : t);
        var e
    }
    ...
    function E(t) {
        if (t instanceof IDBRequest) return function(t) {
            ...
            return e.then((e => {
                e instanceof IDBCursor && y.set(e, t)
            })).catch((() => {})), b.set(e, t), e
        }(t);
        if (D.has(t)) return D.get(t);
        const e = x(t);
        return e !== t && (D.set(t, e), b.set(e, t)), e
    }
    const I = t => b.get(t);
    ...
    function k(t, e) {
        ...
        const s = e.replace(/FromIndex$/, ""),
            n = e !== s,
            r = O.includes(s);
        if (!(s in (n ? IDBIndex : IDBObjectStore).prototype) || !r && !B.includes(s)) return;
    ...
    class M {
        ...
        async expireEntries(t, e) {
            ...
            const a = [];
            for (const t of r) await s.delete(N, t.id), a.push(t.url);
            return a
        }
        m(t) {
            return this.u + "|" + j(t)
    ...
    class S {
        constructor(t, e = {}) {
            this.v = !1, this.D = !1, this.q = e.maxEntries, this.R = e.maxAgeSeconds, this.I = e.matchOptions, this.u = t, this.B = new M(t)
        }
        async expireEntries() {
            if (this.v) return void(this.D = !0);
            this.v = !0;
            const t = this.R ? Date.now() - 1e3 * this.R : 0,
                e = await this.B.expireEntries(t, this.q),
                s = await self.caches.open(this.u);
            for (const t of e) await s.delete(t, this.I);
            this.v = !1, this.D && (this.D = !1, l(this.expireEntries()))
        }
        async updateTimestamp(t) {
            await this.B.setTimestamp(t, Date.now())
        }
        async isURLExpired(t) {
            if (this.R) {
                const e = await this.B.getTimestamp(t),
                    s = Date.now() - 1e3 * this.R;
                return void 0 === e || e < s
            }
            return !1
        }
        async delete() {
            this.D = !1, await this.B.expireEntries(1 / 0)
        }
    }
    ...
    class A {
        constructor(t, e) {
            this.O = {}, Object.assign(this, e), this.event = e.event, this.C = t, this._ = new U, this.k = [], this.N = [...t.plugins], this.j = new Map;
            for (const t of this.N) this.j.set(t, {});
            this.event.waitUntil(this._.promise)
        }
        async fetch(t) {
            const {
                event: e
        ...
        async cacheMatch(t) {
            ...
            const {
                cacheName: n,
                matchOptions: r
            } = this.C, i = await this.getCacheKey(e, "read"), a = Object.assign(Object.assign({}, r), {
                cacheName: n
            });
        ...
        async cachePut(t, e) {
            ...
            const {
                cacheName: c,
                matchOptions: h
            } = this.C, u = await self.caches.open(c), f = this.hasCallback("cacheDidUpdate"), l = f ? await async function(t, e, s, n) {
                const r = T(e.url, s);
                ...
                const i = Object.assign(Object.assign({}, n), {
                    ...
                    a = await t.keys(e, i);
                for (const e of a)
                    if (r === T(e.url, s)) return t.match(e, n)
        ...
        async getCacheKey(t, e) {
            ...
            if (!this.O[s]) {
                let n = t;
                for (const t of this.iterateCallbacks("cacheKeyWillBeUsed")) n = W(await t({
                    mode: e,
                    request: n,
                    event: this.event,
                    params: this.params
    ...
    class P {
        ...
        async S(t, e, n) {
            ...
            try {
                if (r = await this.U(e, t), !r || "error" === r.type) throw new s("no-response", {
                    url: e.url
                })
            } catch (s) {
    ...
    t.CacheFirst = class extends P {
        ...
        constructor(t = {}) {
            ...
            }, this.P = t, this.R = t.maxAgeSeconds, this.L = new Map, t.purgeOnQuotaError && function(t) {
                w.add(t)
        ...
        A(t) {
            ...
            let e = this.L.get(t);
            return e || (e = new S(t, this.P), this.L.set(t, e)), e
        }
        W(t) {
            if (!this.R) return !0;
            const e = this.F(t);
            if (null === e) return !0;
            return e >= Date.now() - 1e3 * this.R
        }
        F(t) {
        ...
        async deleteCacheAndMetadata() {
            for (const [t, e] of this.L) await self.caches.delete(t), await e.delete();
            this.L = new Map
        }
...
//# sourceMappingURL=workbox-668f7341.js.map